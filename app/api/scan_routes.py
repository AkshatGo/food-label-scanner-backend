...
from ..database.gridfs_service import delete_file, get_file, get_image, save_image, save_pdf
...
def validate_image(file: UploadFile, image_bytes: bytes):
    ...
    if supplied_type not in ("", "application/octet-stream", detected_type):
        raise HTTPException(status_code=415, detail="The uploaded file type does not match its image content")

    # Browser file inputs sometimes omit the MIME type or send octet-stream.
...
async def scan_product(file: UploadFile = File(...)):
    ...
    content_type = validate_image(file, image_bytes)
    ...
    image_id = save_image(image_bytes, file.filename, content_type)
    document_inserted = False
    pdf_id = None

    try:
        raw_ocr = extract_text(image_bytes)
        reconstruction = nlp_reconstruct(raw_ocr["text"])
        cleaned_text = reconstruction["human_readable_text"]
        ocr_result = {
            **raw_ocr,
            "raw_text": raw_ocr["text"],
            "text": cleaned_text,
            "cleaned_text": cleaned_text,
            "human_readable_text": reconstruction["human_readable_text"],
            "nutrition_table_detected": reconstruction["nutrition_table_detected"],
        }
        analysis = analyze_enhanced(cleaned_text)
        ...
        pdf_id = save_pdf(
            create_report_pdf(document, image_bytes),
            f"{scan_id}.pdf",
        )
        ...
        if document_inserted:
            scans_collection.delete_one({"scan_id": scan_id})
    ...
    return _json_safe(
        {
            ...
            "scan_id": scan_id,
            "filename": file.filename,
            "message": "Image uploaded, analyzed, and report generated",
            "ocr": ocr_result,
            **analysis,
...
def list_scans(limit: int = Query(20, ge=1, le=100)):
    scans = scans_collection.find().sort("uploaded_at", -1).limit(limit)
    return {"scans": [_json_safe(scan) for scan in scans]}


@router.get("/scans/{scan_id}")
def get_scan(scan_id: str):
    return _json_safe(_find_scan(scan_id))
...
def get_report(scan_id: str):
    ...
    try:
        ...
        if image.get("gridfs_id"):
            try:
                img_stream = get_image(image["gridfs_id"])
                image_bytes = img_stream.read()
            except Exception:
                pass
        pdf_bytes = create_report_pdf(scan, image_bytes)
        return Response(
            content=pdf_bytes,
        ...
        pdf_id = scan.get("pdf", {}).get("gridfs_id")
        if not pdf_id:
            raise HTTPException(status_code=404, detail="Report not found")
        try:
            report = get_file(pdf_id)
        except Exception as error:
            raise HTTPException(status_code=404, detail="Report not found") from error
        return StreamingResponse(report, media_type="application/pdf", headers={"Content-Disposition": f'inline; filename="{scan_id}.pdf"'})
...
def get_scan_image(scan_id: str):
    ...
    image = scan.get("image", {})
    try:
        stored_image = get_image(image["gridfs_id"])
    except Exception as error:
        raise HTTPException(status_code=404, detail="Image not found") from error
    return Response(content=stored_image.read(), media_type=image.get("content_type", "application/octet-stream"))
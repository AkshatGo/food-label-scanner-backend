...
def test_sideways_table_uses_rotated_grid_pass():
    """A sideways mobile label still returns the visible nutrition rows."""
    from pathlib import Path
    fixture = Path("/tmp/codex-clipboard-KrgfLj.png")
    if not fixture.exists():
        pytest.skip("interactive mobile fixture is unavailable")
    result = ocr_service.extract_text(fixture.read_bytes())
    nutrition = extract_nutrition(nlp_reconstruct(result["text"])["human_readable_text"])
